(() => {
  try {
    ...
    const stateKey = () => `monecole_v108_planning_${schoolSlug()}`;
    const permissionKey = () => `monecole_v108_timetable_policy_${schoolSlug()}`;
    const defaultState = () => ({
      active: "planning",
    ...
    const scanCatalog = state => {
      ...
      Object.values(state.sweep || {}).forEach(days => Object.values(days || {}).forEach(list => (list || []).forEach(name => addChoice(students, name))));
      ...
      const visit = (value, context = "") => {
        ...
        if (/eleves?|[eé]l[eè]ves?|students?/.test(ctx)) addChoice(students, value.nom_complet || value.nomComplet || value.fullName || [value.prenom || value.prénom || value.firstName || "", value.nom || value.lastName || ""].filter(Boolean).join(" "));
        ...
        const className = value.classe_nom || value.nom_classe || value.classeName || value.classe || "";
        ...
        if (full) {
          ...
          if (/eleve|élève|student/.test(role) || value.matricule || value.parent || value.tuteur || value.classe_id || className) addChoice(students, full);
        ...
        Object.entries(value).forEach(([key, child]) => {
          ...
          if (typeof child === "string") {
            ...
            if (/eleve|élève|student/.test(k)) addChoice(students, child);
          } else {
            visit(child, key);
          }
        });
      ...
      return `
        ...
        <section class="me-v101-card">
          ...
          <div class="me-v101-form">
            ...
            <div class="me-v101-field"><label>Matière</label><input list="me-v108-subjects" data-course-subject value="${escapeHtml(editing?.subject || "")}" placeholder="Ex : Mathématiques"></div>
            <div class="me-v101-field"><label>Professeur</label><input list="me-v108-professors" data-course-professor value="${escapeHtml(editing?.professor || defaultProfessor)}" placeholder="Nom du professeur"></div>
            ...
            <article class="me-v101-day" data-sweep-day="${day}">
              ...
              <div class="me-v101-day-body">
                <div class="me-v101-form" style="grid-template-columns:minmax(0,1fr) auto">
                  <div class="me-v101-field"><label>Élève</label><input list="me-v108-students" data-sweep-name placeholder="Nom ou prénom"></div>
      ...
      return `
        <section class="me-v101-card">
          ...
          <div class="me-v101-form">
            ...
            <div class="me-v101-field"><label>Matière</label><input list="me-v108-subjects" data-subject-matter value="${escapeHtml(editing?.matter || "")}" placeholder="Ex : Sciences"></div>
    ...
    const renderApp = root => {
      ...
      root.innerHTML = `
        <section class="me-v101-planning-page">
          <datalist id="me-v108-classes">${optionsHtml(classes)}</datalist>
          <datalist id="me-v108-subjects">${optionsHtml(catalog.subjects)}</datalist>
          <datalist id="me-v108-professors">${optionsHtml(catalog.professors)}</datalist>
          <datalist id="me-v108-students">${optionsHtml(catalog.students)}</datalist>
    ...
    console.warn("MonEcole planning v108", error);
  }
})();
