(() => {
  try {
    ...
    const ensurePedagoStyle = () => {
      if (document.getElementById("me-pedagogie-pro-v100-style")) return;
      const style = el("style");
      style.id = "me-pedagogie-pro-v100-style";
      style.textContent = `
    ...
    const cleanupPlanningTools = () => {
      document.getElementById("me-pedago-tools-persistent")?.remove();
      all("#me-pedago-panels,.me-pedago-tools-host", document.body).forEach(node => node.remove());
      all(".me-timetable-page,.me-planning-block,.me-cahier-block,.me-timetable-title,.me-timetable-subtitle", document.body)
        .forEach(node => node.classList.remove("me-timetable-page", "me-planning-block", "me-cahier-block", "me-timetable-title", "me-timetable-subtitle", "is-hidden"));
    };
    const page = () => {
      const main = document.querySelector(".me-main") || document.body;
      const onPlanningNav = sidebarPlanningActive();
      ...
      if (dashboardVisible(main)) return null;
      const marker = all(".me-timetable-toolbar,.me-timetable-day,.me-timetable-cahier,.me-timetable-permission,.me-timetable-lock-banner,strong,h1,h2,h3,section,div", main).find(node => {
        if (node.closest("#me-pedago-panels,.me-pedago-tools")) return false;
        const body = txt(node);
        return /^Note d’aide\s+—\s+(Emploi du temps|Planning)/i.test(body)
          || /^Gestion de l'emploi du temps\b/i.test(body)
          || /Cahier de texte/i.test(body)
      ...
      if (!marker && onPlanningNav && !dashboardVisible(main)) return main.querySelector(".me-page") || main;
    ...
    const renderSubjects = (panel, key) => {
      const list = read(key, []);
      const target = panel.querySelector("[data-subject-list]");
      if (!target) return;
      target.innerHTML = list.length ? "" : '<div class="me-pedago-item"><small>Aucun sujet préparé pour le moment.</small></div>';
      list.forEach((item, index) => {
        const row = el("div", "me-pedago-item", `
          <strong>${escapeHtml(item.type || "Sujet")} · ${escapeHtml(item.title || "Sujet sans titre")}</strong>
          <small>${escapeHtml(item.subject || "Matière")} · ${escapeHtml(dateFr(item.date) || "")}</small>
          <div class="me-pedago-actions">
            <button class="me-pedago-btn" data-print-subject="${index}">Imprimer</button>
            <button class="me-pedago-btn danger" data-delete-subject="${index}">Supprimer</button>
          </div>
        `);
        target.appendChild(row);
      });
      target.onclick = event => {
        const print = event.target.closest("[data-print-subject]");
        const del = event.target.closest("[data-delete-subject]");
        if (print) {
          const item = list[Number(print.dataset.printSubject)];
          printHtml(`${item.type || "Sujet"} - ${item.title || selectedClass(page())}`, `
            <p><b>Classe :</b> ${escapeHtml(selectedClass(page()))}</p>
            <p><b>Matière :</b> ${escapeHtml(item.subject || "")}</p>
        ...
        if (del) {
          list.splice(Number(del.dataset.deleteSubject), 1);
          write(key, list);
          renderSubjects(panel, key);
        }
      };
    };
    const ensurePedagoPanels = root => {
      let wrap = root.querySelector("#me-pedago-panels");
      if (!wrap) {
        wrap = el("div");
        wrap.id = "me-pedago-panels";
        wrap.innerHTML = `
          <section class="me-pedago-panel me-v89-sweep-panel">
            <h3>Liste de balayage</h3>
          </section>
          <section class="me-pedago-panel me-sujet-pro-panel">
            <h3>Préparer un sujet</h3>
            <p>Le professeur prépare ses sujets, les conserve sur l'appareil et peut les imprimer proprement.</p>
            <div class="me-pedago-grid">
              ...
              <div class="me-pedago-field">
                <label>Matière</label>
                <input data-subject-subject placeholder="Ex : Science">
              </div>
              <div class="me-pedago-field">
                <label>Titre</label>
                <input data-subject-title placeholder="Ex : Les fractions">
              </div>
              <div class="me-pedago-field">
                <label>Date</label>
                <input data-subject-date type="date">
              </div>
              <div class="me-pedago-field" style="grid-column:1/-1">
                <label>Consignes et questions</label>
                <textarea data-subject-content placeholder="Rédigez le sujet à imprimer..."></textarea>
              </div>
              <div class="me-pedago-field" style="grid-column:1/-1">
                <label>Corrigé / barème facultatif</label>
                <textarea data-subject-correction placeholder="Corrigé réservé au professeur..."></textarea>
              </div>
            </div>
            <div class="me-pedago-actions">
              <button class="me-pedago-btn primary" data-subject-save>Enregistrer le sujet</button>
            </div>
            <div class="me-pedago-list" data-subject-list></div>
          </section>
        `;
        const anchor = root.querySelector(".me-timetable-cahier")
          || root.querySelector(".me-timetable-days-grid")
          || root.querySelector(".me-timetable-day:last-of-type")
          || root.lastElementChild;
        if (anchor && anchor.parentElement === root) anchor.insertAdjacentElement("afterend", wrap);
        else root.appendChild(wrap);
      }
      const subjectPanel = wrap.querySelector(".me-sujet-pro-panel");
      const key = `${baseKey(root)}_sujets`;
      if (subjectPanel && subjectPanel.dataset.subjectKey !== key) {
        subjectPanel.dataset.subjectKey = key;
        subjectPanel.querySelector("[data-subject-save]").onclick = () => {
          const item = {
            type: subjectPanel.querySelector("[data-subject-type]").value,
            subject: subjectPanel.querySelector("[data-subject-subject]").value.trim(),
            title: subjectPanel.querySelector("[data-subject-title]").value.trim(),
            date: subjectPanel.querySelector("[data-subject-date]").value,
            content: subjectPanel.querySelector("[data-subject-content]").value.trim(),
            correction: subjectPanel.querySelector("[data-subject-correction]").value.trim(),
            createdAt: new Date().toISOString()
          };
          if (!item.content) {
            appMessage("Rédigez d'abord le contenu du sujet.");
            return;
          }
          const list = read(key, []);
          list.unshift(item);
          write(key, list);
          subjectPanel.querySelectorAll("input,textarea").forEach(field => { field.value = ""; dispatch(field); });
          renderSubjects(subjectPanel, key);
          appMessage("Sujet enregistré. Vous pouvez l'imprimer depuis la liste.");
        };
      }
      if (subjectPanel) renderSubjects(subjectPanel, key);
    };
    ...
    const ensurePedagoTools = root => {
      ...
      nav = el("div", "me-pedago-tools", `
        <div>
          <strong>Planning scolaire</strong>
    ...
    const enhance = () => {
      ...
      const root = page();
      if (!root) {
        cleanupPlanningTools();
        return;
      }
      lastPlanningSeenAt = Date.now();
      renameTimetableLabels(root);
      improveDates(root);
      markTimetableStructure(root);
      ensurePedagoPanels(root);
