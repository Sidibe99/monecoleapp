(() => {
  try {
    ...
    const stateKey = () => `monecole_v111_planning_${schoolSlug()}`;
    const permissionKey = () => `monecole_v111_timetable_policy_${schoolSlug()}`;
    const defaultState = () => ({
      active: "planning",
    ...
    const legacyState = () => read(`monecole_v110_planning_${schoolSlug()}`, read(`monecole_v109_planning_${schoolSlug()}`, read(`monecole_v108_planning_${schoolSlug()}`, read(`monecole_v107_planning_${schoolSlug()}`, read(`monecole_v106_planning_${schoolSlug()}`, read(`monecole_v105_planning_${schoolSlug()}`, read(`monecole_v104_planning_${schoolSlug()}`, read(`monecole_v103_planning_${schoolSlug()}`, read(`monecole_v102_planning_${schoolSlug()}`, read(`monecole_v101_planning_${schoolSlug()}`, null))))))))));
    const getState = () => ({ ...defaultState(), ...(legacyState() || {}), ...read(stateKey(), {}) });
    const setState = state => write(stateKey(), state);
    const dateFr = value => {
      const match = String(value || "").match(/^(\d{4})-(\d{2})-(\d{2})$/);
      return match ? `${match[3]}/${match[2]}/${match[1]}` : String(value || "");
    };
    const simpleDate = value => {
    ...
    const schoolTitle = () => {
      ...
      const lines = String(sidebar?.innerText || sidebar?.textContent || "")
        ...
        .map(line => line.trim())
        .filter(Boolean);
      return lines.find(line => line && !/MonEcole|Connecté|ADMINISTRATEUR|DIRECTEUR|PROFESSEUR|SECRÉTAIRE|SECRETAIRE|Synchronisé|Actualiser/i.test(line)) || "MonEcole";
    };
    ...
    const addChoice = (set, value) => {
      const item = cleanChoice(value);
      if (item.length < 2 || item.length > 70) return;
      if (/^(classe|matiere|matière|professeur|enseignant|eleve|élève|undefined|null)$/i.test(item)) return;
      set.add(item);
    };
    const isValidClassName = value => {
      const item = cleanChoice(value);
      if (item.length < 2 || item.length > 36) return false;
      if (/^(classe|class|undefined|null|aucune|none)$/i.test(item)) return false;
      if (/^\d+$/.test(item)) return false;
      if (/^[0-9\s.,_-]+$/.test(item)) return false;
      if (/^[a-f0-9-]{8,}$/i.test(item)) return false;
      if (/^\d{4}-\d{2}-\d{2}/.test(item)) return false;
      if (/(fcfa|gnf|cfa|montant|solde|telephone|téléphone|email|adresse|matricule|identifiant|password|mot de passe)/i.test(item)) return false;
      return /[a-zA-ZÀ-ÿ]/.test(item);
    };
    const addClassChoice = (set, value) => {
      const item = cleanChoice(value);
      if (isValidClassName(item)) set.add(item);
    };
    const scanCatalog = state => {
      const classes = new Set();
      addClassChoice(classes, state.selectedClass);
      const subjects = new Set();
      const professors = new Set();
      const students = new Set();
      const classStudents = new Map();
      const info = userInfo();
      const teacherSubjectHints = new Set();
      const addStudentForClass = (className, name) => {
        const item = cleanChoice(name);
        const classKey = norm(className);
        if (!item || item.length < 2 || !isValidClassName(className)) return;
        if (!classStudents.has(classKey)) classStudents.set(classKey, new Set());
        classStudents.get(classKey).add(item);
      };
      state.courses.forEach(course => {
        addClassChoice(classes, course.className);
        addChoice(subjects, course.subject);
        addChoice(professors, course.professor);
      });
      state.cahiers.forEach(item => addClassChoice(classes, item.className));
      Object.entries(state.sweep || {}).forEach(([className, days]) => {
        addClassChoice(classes, className);
        Object.values(days || {}).forEach(list => (list || []).forEach(name => addStudentForClass(className, name)));
      });
      state.subjects.forEach(item => {
        addClassChoice(classes, item.className);
        addChoice(subjects, item.matter);
        addChoice(professors, item.teacher);
      });
      const seen = new WeakSet();
      const objectName = value => value?.nom || value?.name || value?.libelle || value?.libellé || value?.titre || value?.label || "";
      const visit = (value, context = "", inheritedClass = "") => {
        if (!value || typeof value !== "object") return;
        if (seen.has(value)) return;
        seen.add(value);
        if (Array.isArray(value)) {
          value.forEach(item => visit(item, context, inheritedClass));
          return;
        }
        const ctx = norm(context);
        if (/classes?|classe/.test(ctx)) addClassChoice(classes, objectName(value));
        if (/matieres?|mati[eè]res?|subjects?|disciplines?/.test(ctx)) addChoice(subjects, objectName(value));
        if (/personnel|prof|enseign|teachers?/.test(ctx)) addChoice(professors, value.nom_complet || value.nomComplet || value.fullName || [value.prenom || value.prénom || value.firstName || "", value.nom || value.lastName || ""].filter(Boolean).join(" "));
        const first = value.prenom || value.prénom || value.firstName || value.firstname || "";
        const last = value.nom || value.lastName || value.lastname || "";
        const full = value.nom_complet || value.nomComplet || value.fullName || [first, last].filter(Boolean).join(" ");
        const role = norm(value.role || value.fonction || value.type || "");
        const className = value.classe_nom || value.nom_classe || value.classeName || value.classe || (/classes?|classe/.test(ctx) ? objectName(value) : "") || inheritedClass || "";
        const subjectName = value.matiere_nom || value.matière_nom || value.nom_matiere || value.nom_matière || value.matiere || value.matière || value.subject || "";
        if (className) addClassChoice(classes, className);
        if (subjectName) addChoice(subjects, subjectName);
        if (full) {
          if (/prof|enseign/.test(role) || value.matiere_id || value.matière_id || value.matieres || value.matières) addChoice(professors, full);
          if (/eleve|élève|student/.test(role) || value.matricule || value.parent || value.tuteur || value.classe_id || className) addStudentForClass(className, full);
          if (/PROFESSEUR/.test(info.role) && norm(full) === norm(info.name)) {
            addChoice(teacherSubjectHints, subjectName);
            Object.entries(value).forEach(([key, child]) => {
              const k = norm(key);
              if (!/matiere|matière|subject|discipline/.test(k)) return;
              if (typeof child === "string") addChoice(teacherSubjectHints, child);
              if (Array.isArray(child)) child.forEach(item => {
                if (typeof item === "string") addChoice(teacherSubjectHints, item);
                else addChoice(teacherSubjectHints, item?.nom || item?.name || item?.libelle || item?.libellé || item?.matiere || item?.matière);
              });
            });
          }
        }
        const nextClass = isValidClassName(className) ? className : inheritedClass;
        Object.entries(value).forEach(([key, child]) => {
          const k = norm(key);
          if (typeof child === "string") {
            if (/classe|class/.test(k)) addClassChoice(classes, child);
            if (/matiere|matière|subject|discipline/.test(k)) addChoice(subjects, child);
            if (/prof|enseign|teacher/.test(k)) addChoice(professors, child);
            if (/eleve|élève|student/.test(k)) addStudentForClass(nextClass, child);
          } else {
            visit(child, key, nextClass);
          }
        });
      };
      for (let i = 0; i < localStorage.length; i += 1) {
        const key = localStorage.key(i);
        if (!key || !/monecole|classe|eleve|élève|student|matiere|matière|prof|personnel|teacher/i.test(key)) continue;
        try {
          visit(JSON.parse(localStorage.getItem(key) || "null"), key);
        } catch {}
      }
      const teacherSubjects = new Set();
      if (/PROFESSEUR/.test(info.role)) {
        teacherSubjectHints.forEach(value => addChoice(teacherSubjects, value));
        state.courses
          .filter(course => norm(course.professor) === norm(info.name))
          .forEach(course => addChoice(teacherSubjects, course.subject));
        state.subjects
          .filter(item => norm(item.teacher) === norm(info.name))
          .forEach(item => addChoice(teacherSubjects, item.matter));
      }
      const selectedStudents = classStudents.get(norm(state.selectedClass));
      if (selectedStudents) selectedStudents.forEach(name => addChoice(students, name));
      const sorted = set => Array.from(set).filter(Boolean).sort((a, b) => a.localeCompare(b, "fr"));
      return {
        classes: sorted(classes),
        subjects: sorted(teacherSubjects.size ? teacherSubjects : subjects),
        professors: sorted(professors),
        students: sorted(students)
      };
    };
    const optionsHtml = values => values.map(value => `<option value="${escapeHtml(value)}"></option>`).join("");
    ...
    const canEditSubject = item => {
      const info = userInfo();
      if (!/PROFESSEUR/.test(info.role)) return true;
      return item?.teacher && norm(item.teacher) === norm(info.name);
    };
    const teacherSubjectSet = state => {
      const info = userInfo();
      const set = new Set();
      if (!/PROFESSEUR/.test(info.role)) return set;
      state.courses
        .filter(course => norm(course.professor) === norm(info.name))
        .forEach(course => course.subject && set.add(norm(course.subject)));
      state.subjects
        .filter(item => norm(item.teacher) === norm(info.name))
        .forEach(item => item.matter && set.add(norm(item.matter)));
      return set;
    };
    const isTeacherCourse = (state, course) => {
      const info = userInfo();
      if (!/PROFESSEUR/.test(info.role)) return true;
      if (course?.professor && norm(course.professor) === norm(info.name)) return true;
      const subjects = teacherSubjectSet(state);
      return Boolean(course?.subject && subjects.has(norm(course.subject)));
    };
    const visibleCoursesForClass = state => {
      const base = coursesForClass(state);
      const info = userInfo();
      if (!/PROFESSEUR/.test(info.role)) return base;
      return state.timetableScope === "all" ? base : base.filter(course => isTeacherCourse(state, course));
    };
    const lessonCoursesForClass = state => {
      const base = coursesForClass(state);
      const info = userInfo();
      if (!/PROFESSEUR/.test(info.role)) return base;
      return base.filter(course => canEditLesson(course));
    };
    const lessonEntriesForClass = state => {
      const info = userInfo();
      return state.cahiers
        .filter(item => norm(item.className) === norm(state.selectedClass))
        .filter(item => {
          if (!/PROFESSEUR/.test(info.role)) return true;
          return canEditLesson(state.courses.find(course => course.id === item.courseId));
        });
    };
    ...
    const ensureStyle = () => {
      ...
      style.textContent = `
        .me-v101-planning-page{width:100%;max-width:none;margin:0;color:#0f172a;padding:10px;border-radius:22px;background:linear-gradient(180deg,#f8fbff 0%,#eef6ff 42%,#f8fafc 100%)}
        .me-v101-head{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;margin:0 0 18px;padding:24px;border:1px solid #bfdbfe;border-radius:20px;background:linear-gradient(135deg,#ffffff 0%,#eef6ff 55%,#f8fafc 100%);box-shadow:0 18px 48px rgba(15,23,42,.1)}
        ...
        .me-v101-tabs{display:flex;gap:10px;flex-wrap:wrap;margin:0 0 18px;padding:12px;border:1px solid #dbeafe;border-radius:18px;background:rgba(255,255,255,.92);box-shadow:0 14px 34px rgba(15,23,42,.07);backdrop-filter:blur(10px)}
        ...
        .me-v101-card{border:1px solid #dbeafe;border-radius:18px;background:rgba(255,255,255,.96);padding:18px;margin:0 0 18px;box-shadow:0 14px 36px rgba(15,23,42,.07)}
        ...
        .me-v101-filterbar{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin:0 0 16px;padding:13px 14px;border:1px solid #dbeafe;border-radius:16px;background:#fff}
        .me-v101-filterbar strong{display:block;color:#0f172a;font-weight:950}
        .me-v101-filterbar span{display:block;color:#64748b;font-weight:750;font-size:13px;margin-top:2px}
        .me-v101-segment{display:flex;gap:8px;flex-wrap:wrap}
        .me-v101-segment button{border:1px solid #cbd5e1;background:#fff;color:#475569;border-radius:999px;min-height:40px;padding:0 14px;font:inherit;font-weight:950;cursor:pointer}
        .me-v101-segment button.is-active{border-color:#1d4ed8;background:#1d4ed8;color:#fff}
        ...
        @media print{body *{visibility:hidden!important}.me-v101-print,.me-v101-print *{visibility:visible!important}.me-v101-print{display:block!important;position:absolute;inset:0;padding:28px;background:#fff;color:#111;font-family:Arial,sans-serif}.me-v101-print-header{display:grid;grid-template-columns:1fr auto;gap:14px;align-items:start;border-bottom:3px solid #1d4ed8;padding-bottom:14px;margin-bottom:18px}.me-v101-print-header h1{margin:0;font-size:22px}.me-v101-print-header p{margin:4px 0 0;color:#334155}.me-v101-print-meta{text-align:right;font-size:12px;color:#475569}.me-v101-print h2{font-size:16px;margin:18px 0 8px}.me-v101-print table{width:100%;border-collapse:collapse}.me-v101-print th,.me-v101-print td{border:1px solid #94a3b8;padding:8px;text-align:left;vertical-align:top}.me-v101-print th{background:#eff6ff}}
      `;
      document.head.appendChild(style);
    };
    const classList = state => {
      const set = new Set();
      addClassChoice(set, state.selectedClass);
      state.courses.forEach(course => addClassChoice(set, course.className));
      state.cahiers.forEach(item => addClassChoice(set, item.className));
    ...
    const printHtml = (title, body, meta = {}) => {
      ...
      zone.innerHTML = `
        <div class="me-v101-print-header">
          <div>
            <h1>${escapeHtml(title)}</h1>
            <p>${escapeHtml(schoolTitle())}</p>
            ${meta.subtitle ? `<p>${escapeHtml(meta.subtitle)}</p>` : ""}
          </div>
          <div class="me-v101-print-meta">
            <div>Classe : ${escapeHtml(meta.className || "Toutes")}</div>
            <div>Année scolaire : ${escapeHtml(meta.schoolYear || state.schoolYear || schoolYearLabel())}</div>
            <div>Imprimé le ${escapeHtml(todayFr())}</div>
          </div>
        </div>
        ${body}
      `;
    ...
    const courseOptions = (state, courses = lessonCoursesForClass(state)) => courses
      .map(course => `<option value="${escapeHtml(course.id)}">${escapeHtml(`${course.className || "Classe"} · ${course.day} ${course.start || ""}-${course.end || ""} · ${course.subject || "Cours"} · ${course.professor || "Professeur"}`)}</option>`)
      .join("");
    const renderPlanningPanel = state => {
      ...
      const courses = visibleCoursesForClass(state);
      const info = userInfo();
      const isTeacher = /PROFESSEUR/.test(info.role);
      const defaultProfessor = /PROFESSEUR/.test(info.role) ? info.name : "";
      return `
        ...
        <div class="me-v101-filterbar">
          <div>
            <strong>Affichage de l'emploi du temps</strong>
          </div>
          <div class="me-v101-segment">
            ${isTeacher ? `<button type="button" class="${state.timetableScope !== "all" ? "is-active" : ""}" data-scope="mine">Mes cours</button><button type="button" class="${state.timetableScope === "all" ? "is-active" : ""}" data-scope="all">Tout afficher</button>` : ""}
            <button type="button" data-print-timetable>Imprimer l'emploi du temps</button>
          </div>
        </div>
        <section class="me-v101-card">
          ...
          <div class="me-v101-form">
            ...
            <div class="me-v101-field"><label>Matière</label><input list="me-v111-subjects" data-course-subject value="${escapeHtml(editing?.subject || "")}" placeholder="Ex : Mathématiques"></div>
            <div class="me-v101-field"><label>Professeur</label><input list="me-v111-professors" data-course-professor value="${escapeHtml(editing?.professor || defaultProfessor)}" placeholder="Nom du professeur"></div>
    ...
    const renderCahierPanel = state => {
      const courses = lessonCoursesForClass(state);
      const selectedCourse = courses[0];
      const entries = lessonEntriesForClass(state);
      const allowed = selectedCourse ? canEditLesson(selectedCourse) : false;
      const info = userInfo();
      const classCourses = coursesForClass(state);
      const noTeacherCourse = /PROFESSEUR/.test(info.role) && !courses.length;
      const noClassCourse = !classCourses.length;
      const message = allowed
        ? "Professeur programmé détecté : saisie autorisée pour ce cours."
        : noTeacherCourse
          ? "Aucun cours n'est programmé pour votre compte dans cette classe."
          : noClassCourse
            ? "Aucun cours n'est encore programmé pour la classe affichée."
            : "Lecture seule : seul le professeur programmé sur le cours peut saisir, modifier ou supprimer le cahier de texte.";
      return `
        <div class="me-v101-lock ${allowed ? "me-v101-ok" : ""}">${message}</div>
        <section class="me-v101-card">
          <h2>Cahier de texte</h2>
          <div class="me-v101-form">
            <div class="me-v101-field"><label>Cours</label><select data-lesson-course ${courses.length ? "" : "disabled"}>${courseOptions(state, courses) || '<option value="">Aucun cours programmé</option>'}</select></div>
            <div class="me-v101-field"><label>Date</label><input data-lesson-date inputmode="numeric" maxlength="10" value="${escapeHtml(todayFr())}" placeholder="JJ/MM/AAAA"></div>
          ...
          ${courses.length ? "" : '<div class="me-v101-actions"><button class="me-v101-btn" type="button" data-open-planning>Programmer un cours</button></div>'}
          <div class="me-v101-actions">
    ...
    const renderSweepPanel = state => {
      ...
      return `
        <section class="me-v101-card">
          <h2>Liste de balayage</h2>
          <div class="me-v101-actions"><button class="me-v101-btn" data-print-sweep>Imprimer la fiche</button></div>
        </section>
            ...
            <article class="me-v101-day" data-sweep-day="${day}">
              ...
              <div class="me-v101-day-body">
                <div class="me-v101-form" style="grid-template-columns:minmax(0,1fr) auto">
                  <div class="me-v101-field"><label>Élève</label><input list="me-v111-students" data-sweep-name placeholder="Nom ou prénom"></div>
    ...
    const renderSubjectsPanel = state => {
      ...
      const editing = state.subjects.find(item => item.id === state.editingSubjectId);
      return `
        <section class="me-v101-card">
          <h2>Préparer un sujet</h2>
          <div class="me-v101-form">
            <div class="me-v101-field"><label>Type</label><select data-subject-type>${["Evaluation", "Composition", "Test", "Devoir maison"].map(type => `<option ${editing?.type === type ? "selected" : ""}>${type}</option>`).join("")}</select></div>
            <div class="me-v101-field"><label>Matière</label><input list="me-v111-subjects" data-subject-matter value="${escapeHtml(editing?.matter || "")}" placeholder="Ex : Sciences"></div>
            <div class="me-v101-field"><label>Titre</label><input data-subject-title value="${escapeHtml(editing?.title || "")}" placeholder="Ex : Les fractions"></div>
            <div class="me-v101-field"><label>Date</label><input data-subject-date inputmode="numeric" maxlength="10" value="${escapeHtml(simpleDate(editing?.date || ""))}" placeholder="JJ/MM/AAAA"></div>
            <div class="me-v101-field full"><label>Consignes et questions</label><textarea data-subject-content placeholder="Rédigez le sujet à imprimer...">${escapeHtml(editing?.content || "")}</textarea></div>
            <div class="me-v101-field full"><label>Corrigé / barème facultatif</label><textarea data-subject-correction placeholder="Corrigé réservé au professeur...">${escapeHtml(editing?.correction || "")}</textarea></div>
          </div>
          <div class="me-v101-actions">
            <button class="me-v101-btn primary" data-save-subject>${editing ? "Mettre à jour le sujet" : "Enregistrer le sujet"}</button>
            ${editing ? '<button class="me-v101-btn" data-cancel-subject-edit>Annuler</button>' : ""}
          </div>
        </section>
        <section class="me-v101-card">
          ...
          ${subjects.length ? subjects.map(item => `
            <div class="me-v101-item">
              ...
              <div class="me-v101-actions">
                <button class="me-v101-btn" data-edit-subject="${escapeHtml(item.id)}">Modifier</button>
    ...
    const renderApp = root => {
      ensureStyle();
      const state = getState();
      if (!["planning", "cahier"].includes(state.active)) state.active = "planning";
      if (!state.schoolYear) {
        state.schoolYear = schoolYearLabel();
        setState(state);
      }
      const catalog = scanCatalog(state);
      const classes = Array.from(new Set([...(catalog.classes || []), ...classList(state)]))
        .filter(isValidClassName)
        .sort((a, b) => a.localeCompare(b, "fr"));
      if (!isValidClassName(state.selectedClass) || !classes.some(item => norm(item) === norm(state.selectedClass))) {
        state.selectedClass = classes[0];
        setState(state);
      }
      if (/PROFESSEUR/.test(userInfo().role) && state.active === "cahier" && !lessonCoursesForClass(state).length) {
        const teacherClass = classes.find(className =>
          state.courses.some(course => norm(course.className) === norm(className) && canEditLesson(course))
        );
        if (teacherClass && norm(teacherClass) !== norm(state.selectedClass)) {
      ...
      root.innerHTML = `
        <section class="me-v101-planning-page">
          <datalist id="me-v111-classes">${optionsHtml(classes)}</datalist>
          <datalist id="me-v111-subjects">${optionsHtml(catalog.subjects)}</datalist>
          <datalist id="me-v111-professors">${optionsHtml(catalog.professors)}</datalist>
          <datalist id="me-v111-students">${optionsHtml(catalog.students)}</datalist>
          ...
          <section class="me-v101-card">
            <div class="me-v101-form">
              <div class="me-v101-field">
                ...
                <select data-selected-class ${classes.length ? "" : "disabled"}>
                  ${classes.length ? classes.map(name => `<option value="${escapeHtml(name)}" ${norm(name) === norm(state.selectedClass) ? "selected" : ""}>${escapeHtml(name)}</option>`).join("") : '<option>Aucune classe détectée</option>'}
                </select>
              </div>
              <div class="me-v101-field">
                <label>Année scolaire</label>
                <input data-school-year value="${escapeHtml(state.schoolYear || schoolYearLabel())}" placeholder="Ex : 2025-2026">
              </div>
            </div>
          </section>
    ...
    const bindApp = root => {
      ...
      root.querySelector("[data-open-planning]")?.addEventListener("click", () => {
        const state = getState();
      ...
      const schoolYear = root.querySelector("[data-school-year]");
      if (schoolYear) {
        schoolYear.onchange = () => {
          const state = getState();
          state.schoolYear = schoolYear.value.trim() || schoolYearLabel();
          setState(state);
          renderApp(root);
        };
      }
      root.querySelectorAll("[data-lesson-date], [data-subject-date]").forEach(input => {
        input.oninput = () => {
          input.value = simpleDate(input.value);
        };
      });
      ...
      if (saveCourse) saveCourse.onclick = () => {
        ...
        const state = getState();
        if (!isValidClassName(state.selectedClass)) return appMessage("Choisissez d'abord une classe enregistrée.");
        const course = {
      ...
      root.querySelectorAll("[data-scope]").forEach(button => {
        button.onclick = () => {
          const state = getState();
          state.timetableScope = button.dataset.scope;
          setState(state);
          renderApp(root);
        };
      });
      root.querySelector("[data-print-timetable]")?.addEventListener("click", () => {
        const state = getState();
        const courses = visibleCoursesForClass(state);
        const rows = DAYS.map(day => {
          const dayCourses = courses.filter(course => course.day === day).sort((a, b) => String(a.start).localeCompare(String(b.start)));
          return dayCourses.length
            ? dayCourses.map(course => `<tr><th>${escapeHtml(day)}</th><td>${escapeHtml(course.start || "")} - ${escapeHtml(course.end || "")}</td><td>${escapeHtml(course.subject || "Cours")}</td><td>${escapeHtml(course.professor || "")}</td><td>${escapeHtml(course.room || "")}</td></tr>`).join("")
            : `<tr><th>${escapeHtml(day)}</th><td colspan="4">Aucun cours</td></tr>`;
        }).join("");
        printHtml("Emploi du temps", `
          <table>
            <thead><tr><th>Jour</th><th>Horaire</th><th>Matière</th><th>Professeur</th><th>Salle</th></tr></thead>
            <tbody>${rows}</tbody>
          </table>
        `, {
          className: state.selectedClass,
          subtitle: state.timetableScope === "all" ? "Vue complète" : "Vue professeur"
        });
      });
      ...
      root.querySelector("[data-save-lesson]")?.addEventListener("click", () => {
        ...
        const entry = {
          ...
          date: simpleDate(root.querySelector("[data-lesson-date]").value),
      ...
      root.querySelectorAll("[data-add-sweep]").forEach(button => {
        button.onclick = () => {
          ...
          if (!name) return;
          const state = getState();
          const validStudents = scanCatalog(state).students || [];
          if (validStudents.length && !validStudents.some(student => norm(student) === norm(name))) {
            return appMessage("Choisissez un élève de la classe affichée.");
          }
      ...
      root.querySelector("[data-print-sweep]")?.addEventListener("click", () => {
        ...
        printHtml("Liste de balayage", `<table><thead><tr><th>Jour</th><th>Élèves de service</th></tr></thead><tbody>${rows}</tbody></table>`, {
          className: state.selectedClass,
          subtitle: "Élèves de service par jour"
        });
      });
      root.querySelector("[data-save-subject]")?.addEventListener("click", () => {
        const state = getState();
        const info = userInfo();
        const previous = state.subjects.find(subject => subject.id === state.editingSubjectId);
        if (previous && !canEditSubject(previous)) return appMessage("Modification réservée au professeur propriétaire du sujet.");
        const item = {
          id: previous?.id || uid(),
          teacher: previous?.teacher || info.name || "Professeur",
          className: state.selectedClass,
          ...
          date: simpleDate(root.querySelector("[data-subject-date]").value),
        ...
        state.subjects = state.subjects.filter(subject => subject.id !== item.id);
        state.subjects.unshift(item);
        state.editingSubjectId = "";
        setState(state);
        renderApp(root);
      });
      root.querySelector("[data-cancel-subject-edit]")?.addEventListener("click", () => {
        const state = getState();
        state.editingSubjectId = "";
        setState(state);
        renderApp(root);
      });
      root.querySelectorAll("[data-edit-subject]").forEach(button => {
        button.onclick = () => {
          const state = getState();
          const item = state.subjects.find(subject => subject.id === button.dataset.editSubject);
          if (!canEditSubject(item)) return appMessage("Modification réservée au professeur propriétaire du sujet.");
          state.editingSubjectId = button.dataset.editSubject;
          state.active = "sujets";
          setState(state);
          renderApp(root);
        };
      ...
      root.querySelectorAll("[data-delete-subject]").forEach(button => {
        button.onclick = () => {
          ...
          if (!canEditSubject(item)) return appMessage("Action réservée au professeur propriétaire du sujet.");
          state.subjects = state.subjects.filter(subject => subject.id !== button.dataset.deleteSubject);
          if (state.editingSubjectId === button.dataset.deleteSubject) state.editingSubjectId = "";
          setState(state);
          renderApp(root);
        };
      });
      root.querySelectorAll("[data-print-subject]").forEach(button => {
        button.onclick = () => {
          ...
          printHtml(`${item.type} - ${item.title || state.selectedClass}`, `
            <p><b>Matière :</b> ${escapeHtml(item.matter || "")}</p>
            <p><b>Date :</b> ${escapeHtml(dateFr(item.date) || "")}</p>
            <p><b>Professeur :</b> ${escapeHtml(item.teacher || userInfo().name || "")}</p>
          ...
          `, {
            className: item.className || state.selectedClass,
            subtitle: `${item.matter || "Matière"}${item.date ? ` · ${dateFr(item.date)}` : ""}`
          });
        };
      });
    };
    ...
    const ensureSettingsPermission = () => {
      ...
      const box = el("section", "me-v101-permission me-v101-card", `
        ...
        <p>La direction choisit ici les profils autorisés à modifier l'emploi du temps.</p>
        <div class="me-v101-field">
    ...
    const renameNav = () => {
      all("aside button,aside a,nav button,nav a,[role='button']", document.body).forEach(node => {
        if (!/Emploi du temps|Planning\s*&\s*outils|Planning scolaire/i.test(txt(node))) return;
        const walker = document.createTreeWalker(node, NodeFilter.SHOW_TEXT);
        while (walker.nextNode()) {
          walker.currentNode.nodeValue = walker.currentNode.nodeValue
            .replace(/Planning\s*&\s*outils/gi, "Emploi du temps")
            .replace(/Planning scolaire/gi, "Emploi du temps");
        }
      });
    };
    let pending = false;
    ...
    console.warn("MonEcole planning v111", error);
  }
})();
