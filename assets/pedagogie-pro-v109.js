(() => {
  try {
    ...
    const stateKey = () => `monecole_v109_planning_${schoolSlug()}`;
    const permissionKey = () => `monecole_v109_timetable_policy_${schoolSlug()}`;
    const defaultState = () => ({
      active: "planning",
    ...
    const lessonCoursesForClass = state => {
      const base = coursesForClass(state);
      const info = userInfo();
      if (!/PROFESSEUR/.test(info.role)) return base.length ? base : state.courses;
      const teacherCourses = base.filter(course => canEditLesson(course));
      if (teacherCourses.length) return teacherCourses;
      return state.courses.filter(course => canEditLesson(course));
    };
    const lessonEntriesForClass = state => {
      const info = userInfo();
      const classHasCourse = coursesForClass(state).length > 0;
      return state.cahiers
        ...
        .concat(classHasCourse ? [] : state.cahiers
          .filter(item => norm(item.className) !== norm(state.selectedClass))
          .filter(item => {
            if (!/PROFESSEUR/.test(info.role)) return false;
            return canEditLesson(state.courses.find(course => course.id === item.courseId));
          }));
    };
      ...
      return `
        ...
        <section class="me-v101-card">
          ...
          <div class="me-v101-form">
            ...
            <div class="me-v101-field"><label>Matière</label><input list="me-v109-subjects" data-course-subject value="${escapeHtml(editing?.subject || "")}" placeholder="Ex : Mathématiques"></div>
            <div class="me-v101-field"><label>Professeur</label><input list="me-v109-professors" data-course-professor value="${escapeHtml(editing?.professor || defaultProfessor)}" placeholder="Nom du professeur"></div>
    ...
    const renderCahierPanel = state => {
      ...
      const fallbackCourses = !classCourses.length && courses.length;
      const message = allowed
        ...
        : noTeacherCourse
          ...
          : fallbackCourses
            ? "Aucun cours dans la classe affichée : les cours programmés disponibles sont proposés ci-dessous."
            ...
            <article class="me-v101-day" data-sweep-day="${day}">
              ...
              <div class="me-v101-day-body">
                <div class="me-v101-form" style="grid-template-columns:minmax(0,1fr) auto">
                  <div class="me-v101-field"><label>Élève</label><input list="me-v109-students" data-sweep-name placeholder="Nom ou prénom"></div>
      ...
      return `
        <section class="me-v101-card">
          ...
          <div class="me-v101-form">
            ...
            <div class="me-v101-field"><label>Matière</label><input list="me-v109-subjects" data-subject-matter value="${escapeHtml(editing?.matter || "")}" placeholder="Ex : Sciences"></div>
    ...
    const renderApp = root => {
      ...
      root.innerHTML = `
        <section class="me-v101-planning-page">
          <datalist id="me-v109-classes">${optionsHtml(classes)}</datalist>
          <datalist id="me-v109-subjects">${optionsHtml(catalog.subjects)}</datalist>
          <datalist id="me-v109-professors">${optionsHtml(catalog.professors)}</datalist>
          <datalist id="me-v109-students">${optionsHtml(catalog.students)}</datalist>
    ...
    console.warn("MonEcole planning v109", error);
  }
})();
