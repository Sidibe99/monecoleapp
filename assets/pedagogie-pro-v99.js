(() => {
  try {
    ...
    const ensurePedagoStyle = () => {
      if (document.getElementById("me-pedagogie-pro-v99-style")) return;
      const style = el("style");
      style.id = "me-pedagogie-pro-v99-style";
      style.textContent = `
    ...
    const sidebarPlanningActive = () => {
      const active = all("aside button,aside a,nav button,nav a,[role='button']", document.body)
        .find(node => {
          if (!/Planning\s*&\s*outils|Planning scolaire|Emploi du temps/i.test(txt(node))) return false;
          try {
            const bg = getComputedStyle(node).backgroundColor;
            return /rgb\(37,\s*99,\s*235\)|rgb\(26,\s*86,\s*219\)|rgb\(30,\s*64,\s*175\)/.test(bg);
          } catch {
            return false;
          }
        });
      const selected = all("aside button,aside a,nav button,nav a,[aria-current='page'],[data-active='true']", document.body)
        .find(node => /Planning\s*&\s*outils|Planning scolaire|Emploi du temps/i.test(txt(node)));
      return Boolean(active || selected);
    };
    const page = () => {
      ...
      if (!marker && sidebarPlanningActive()) return main.querySelector(".me-page") || main;
    ...
    const enhance = () => {
      ...
      if (!root) {
        ...
        const keepVisible = sidebarPlanningActive() || Date.now() - lastPlanningSeenAt < 2500;
        if (nav) nav.hidden = !keepVisible;
        return;
      }
