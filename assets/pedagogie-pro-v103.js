(() => {
  try {
    ...
    const stateKey = () => `monecole_v103_planning_${schoolSlug()}`;
    const permissionKey = () => `monecole_v103_timetable_policy_${schoolSlug()}`;
    const defaultState = () => ({
      active: "planning",
    ...
    const legacyState = () => read(`monecole_v102_planning_${schoolSlug()}`, read(`monecole_v101_planning_${schoolSlug()}`, null));
    ...
    const scanCatalog = state => {
      const classes = new Set([state.selectedClass || "Classe"]);
      ...
      state.courses.forEach(course => {
        addChoice(classes, course.className);
        addChoice(subjects, course.subject);
        addChoice(professors, course.professor);
      });
      state.cahiers.forEach(item => addChoice(classes, item.className));
      ...
      state.subjects.forEach(item => {
        addChoice(classes, item.className);
      ...
      const visit = (value, context = "") => {
        ...
        if (/classes?|classe/.test(ctx)) addChoice(classes, objectName(value));
        ...
        if (className) addChoice(classes, className);
        ...
        Object.entries(value).forEach(([key, child]) => {
          ...
          if (typeof child === "string") {
            if (/classe|class/.test(k)) addChoice(classes, child);
      ...
      return `
        ...
        <section class="me-v101-card">
          ...
          <div class="me-v101-form">
            ...
            <div class="me-v101-field"><label>Matière</label><input list="me-v103-subjects" data-course-subject value="${escapeHtml(editing?.subject || "")}" placeholder="Ex : Mathématiques"></div>
            <div class="me-v101-field"><label>Professeur</label><input list="me-v103-professors" data-course-professor value="${escapeHtml(editing?.professor || defaultProfessor)}" placeholder="Nom du professeur"></div>
            ...
            <article class="me-v101-day" data-sweep-day="${day}">
              ...
              <div class="me-v101-day-body">
                <div class="me-v101-form" style="grid-template-columns:minmax(0,1fr) auto">
                  <div class="me-v101-field"><label>Élève</label><input list="me-v103-students" data-sweep-name placeholder="Nom ou prénom"></div>
      ...
      return `
        <section class="me-v101-card">
          ...
          <div class="me-v101-form">
            ...
            <div class="me-v101-field"><label>Matière</label><input list="me-v103-subjects" data-subject-matter value="${escapeHtml(editing?.matter || "")}" placeholder="Ex : Sciences"></div>
    ...
    const renderApp = root => {
      ...
      const classes = catalog.classes.length ? catalog.classes : classList(state);
      if ((!state.selectedClass || state.selectedClass === "Classe") && classes.length && classes[0] !== "Classe") {
        state.selectedClass = classes[0];
        setState(state);
      }
      root.innerHTML = `
        <section class="me-v101-planning-page">
          <datalist id="me-v103-classes">${optionsHtml(classes)}</datalist>
          <datalist id="me-v103-subjects">${optionsHtml(catalog.subjects)}</datalist>
          <datalist id="me-v103-professors">${optionsHtml(catalog.professors)}</datalist>
          <datalist id="me-v103-students">${optionsHtml(catalog.students)}</datalist>
          ...
          <section class="me-v101-card">
            <div class="me-v101-form">
              <div class="me-v101-field">
                ...
                <input list="me-v103-classes" data-selected-class value="${escapeHtml(state.selectedClass)}" placeholder="Ex : 6e A">
              </div>
              <div class="me-v101-field">
    ...
    console.warn("MonEcole planning v103", error);
  }
})();
