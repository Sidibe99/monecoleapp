(function () {
  ...
  function isPrimaryLevel(className) {
    ...
    if (/college|lycee|lycee|7eme|8eme|9eme|10eme|11eme|12eme|terminal|seconde|premiere/.test(value)) return false;
    return /maternelle|primaire|annee|1ere|2eme|3eme|4eme|5eme|6eme|cp|ce|cm|moyenne|petite|grande/.test(value) || true;
  }
  ...
  function teacherMatches(currentName, teacherName) {
    ...
    var tokensA = norm(currentName).split(" ").filter(function (part) { return part.length > 1; });
    var tokensB = norm(teacherName).split(" ").filter(function (part) { return part.length > 1; });
  ...
  function ensureNotice(card) {
    ...
    if (!note) {
      ...
      if (title && title.parentElement) {
        title.parentElement.insertAdjacentElement("afterend", note);
      } else {
  ...
  function ensureSessionSelect(card, courses, show) {
    ...
    if (!box) {
      ...
      var target = save && save.parentElement ? save.parentElement : card.firstChild;
