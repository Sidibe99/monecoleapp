(() => {
  try {
    ...
    const applyPedagoVisibility = root => {
      ...
      if (!wrap || !nav) return;
      const target = nav.dataset.activeTool || "cahier";
    ...
    const ensurePedagoTools = root => {
      ...
      nav = el("div", "me-pedago-tools", `
        <div>
          ...
          <span>Choisissez une partie à afficher.</span>
        </div>
