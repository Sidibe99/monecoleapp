(() => {
  try {
    ...
    const ensurePedagoStyle = () => {
      if (document.getElementById("me-pedagogie-pro-v94-style")) return;
      const style = el("style");
      style.id = "me-pedagogie-pro-v94-style";
      style.textContent = `
    ...
    const page = () => {
      ...
      const marker = all(".me-timetable-permission,.me-timetable-lock-banner,strong,h1,h2,h3,section,div", main).find(node => {
        const body = txt(node);
    ...
    const applyPedagoVisibility = root => {
      ...
      if (target === "planning") {
        all(".me-timetable-toolbar,.me-timetable-days-grid,.me-timetable-day,.me-timetable-permission,.me-timetable-lock-banner", root)
    ...
    const ensurePedagoTools = root => {
      ...
      const permission = root.querySelector(".me-timetable-permission,.me-timetable-lock-banner");
      if (permission?.parentElement) {
        permission.insertAdjacentElement("beforebegin", nav);
      } else if (title?.parentElement) {
        title.insertAdjacentElement("beforebegin", nav);
      } else if (help?.parentElement) {
