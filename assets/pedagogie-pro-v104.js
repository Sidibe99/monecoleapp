(() => {
  try {
    ...
    const stateKey = () => `monecole_v104_planning_${schoolSlug()}`;
    const permissionKey = () => `monecole_v104_timetable_policy_${schoolSlug()}`;
    const defaultState = () => ({
      active: "planning",
    ...
    const legacyState = () => read(`monecole_v103_planning_${schoolSlug()}`, read(`monecole_v102_planning_${schoolSlug()}`, read(`monecole_v101_planning_${schoolSlug()}`, null)));
    ...
    const renderPlanningPanel = state => {
      ...
      return `
        ...
        <section class="me-v101-card">
          ...
          <p>Ajoutez les cours avec les suggestions automatiques de classes, matières et professeurs déjà enregistrés.</p>
          <div class="me-v101-form">
            ...
            <div class="me-v101-field"><label>Matière</label><input list="me-v104-subjects" data-course-subject value="${escapeHtml(editing?.subject || "")}" placeholder="Ex : Mathématiques"></div>
            <div class="me-v101-field"><label>Professeur</label><input list="me-v104-professors" data-course-professor value="${escapeHtml(editing?.professor || defaultProfessor)}" placeholder="Nom du professeur"></div>
            ...
            <article class="me-v101-day" data-sweep-day="${day}">
              ...
              <div class="me-v101-day-body">
                <div class="me-v101-form" style="grid-template-columns:minmax(0,1fr) auto">
                  <div class="me-v101-field"><label>Élève</label><input list="me-v104-students" data-sweep-name placeholder="Nom ou prénom"></div>
      ...
      return `
        <section class="me-v101-card">
          ...
          <div class="me-v101-form">
            ...
            <div class="me-v101-field"><label>Matière</label><input list="me-v104-subjects" data-subject-matter value="${escapeHtml(editing?.matter || "")}" placeholder="Ex : Sciences"></div>
    ...
    const renderApp = root => {
      ...
      root.innerHTML = `
        <section class="me-v101-planning-page">
          <datalist id="me-v104-classes">${optionsHtml(classes)}</datalist>
          <datalist id="me-v104-subjects">${optionsHtml(catalog.subjects)}</datalist>
          <datalist id="me-v104-professors">${optionsHtml(catalog.professors)}</datalist>
          <datalist id="me-v104-students">${optionsHtml(catalog.students)}</datalist>
    ...
    const bindApp = root => {
      ...
      root.querySelector("[data-print-sweep]")?.addEventListener("click", () => {
        ...
        printHtml("Liste de balayage", `<p>Classe : ${escapeHtml(state.selectedClass)} · Année scolaire : ${escapeHtml(schoolYearLabel())}</p><table><tbody>${rows}</tbody></table>`);
      });
    ...
    console.warn("MonEcole planning v104", error);
  }
})();
